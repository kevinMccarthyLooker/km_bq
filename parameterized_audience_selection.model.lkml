...
view: audience {
  derived_table: {
    ...
    sql:
select 'a' as audience, 'UK, USA' as country_filter, null as sequence_number_filter, null as browser_filter union all
...
#   and position(coalesce(${audience.country_filter},${events.country}) in ${events.country})>0
#   and position(coalesce(${audience.browser_filter},${events.browser}) in ${events.browser})>0
    sql_on: 1=1
    and position(${events.country} in coalesce(${audience.country_filter},${events.country}) )>0
    and position(${events.browser} in coalesce(${audience.browser_filter},${events.browser}) )>0
