...
#add updated explore example
explore: users {
  #join to any explore, just need to update which field we refer to in YOY
  join: pop_support_users {
    type:cross
    relationship:one_to_one
#     sql_where: ${pop_support_users.analysis_date}<=(select max(${pop_support_users.input__the_date_to_pop}) from {{pop_support_users.input__the_original_source_table._sql}} AS {{pop_support_users.input__the_original_source_table_view_name._sql}});;
  }
}
view: pop_support_users {
  ...
  dimension_group: analysis {
    #Update label or timeframes as desired
#     group_label: "{{users.created_date._label}} (POP)"
    label: "(POP) User Created"
#     timeframes: [date,week,month,quarter,year] #you can update to disable timeframes
  }
